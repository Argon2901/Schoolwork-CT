...
struct expr * expr_create_ID( char* id )
{
	struct expr* e = expr_create(EXPR_ID,0,0);
	e->id = id;
	return e;
}
...
float expr_evaluate( struct expr *e )
{
	...
	switch(e->kind) {
		...
		case EXPR_COS:
		   	...
		   	return cos(r);
		case EXPR_ASSIGN:
			getAddr_symTab(&symtab, e->left->id)->value = (int)r;
			return r;
		case EXPR_ID:
			return getAddr_symTab(&symtab, e->id)->value;
	}
