...
{
  //cprintf("fetching int from addr 0x%x\n", addr);
  //struct proc *curproc = myproc();

//removing safety features for now because they're breaking the new stack layout
...
argptr(int n, char **pp, int size)
{
  ...
  if(argint(n, &i) < 0)
    return -1;
  //removing safety because it's wrong with new stack layout
