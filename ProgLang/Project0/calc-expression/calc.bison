...
%union myunion{
	char* s;
	struct expr* a;
} //Replacing YYSTYPE with my own specification
...
%token TOKEN_SIN
%token TOKEN_COS
%type <a> program assign expr term factor
...
assign: 	expr
		{ $$ = $1; }
	| TOKEN_ID TOKEN_EQUALS expr 
		{ //needs to be fixed, extract id value
			$$ = expr_create(EXPR_ASSIGN, expr_create_ID($1) , $3);
		}
	;
