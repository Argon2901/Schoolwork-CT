...
void CPU::doCycleWork(){
    ...
    switch(state){
        ...
        case fetchI:
            ...
            //printf("working on instruction %5X\n", instruction);
...
CPU::CPU(Memory* memory, Instruction_Memory* iMemory){
    this->memory = memory;
    this->iMemory = iMemory;

    registers.PC = 0;
    registers.rReg = (unsigned char*) malloc(8);
    for(int i = 0; i < 8; i++){
        registers.rReg[i] = 0;
    }
}
...
void CPU::add(unsigned char dReg, unsigned char sReg, unsigned char tReg, unsigned char* completed){
    registers.rReg[dReg] = (char)registers.rReg[sReg] + (char)registers.rReg[tReg];
    registers.PC++;
    state = idle;
}

void CPU::addi(unsigned char dReg, unsigned char sReg, unsigned char immediate, unsigned char* completed){
    registers.rReg[dReg] = (char)registers.rReg[sReg] + (char)immediate;
    *completed = true;
}
...
void CPU::load(unsigned char dReg, unsigned char tReg, unsigned char* completed){
    ...
    memory->memFetch(target, (unsigned int)1, destination, completed);
...
void CPU::store(unsigned char sReg, unsigned char tReg, unsigned char* completed){
    ...
    memory->memStore(target, (unsigned int)1, source, completed);
}
