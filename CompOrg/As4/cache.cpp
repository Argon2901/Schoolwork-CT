...
{ 
    ...
    if(!isActive){
        memory->memFetch(address, count, dataPtr, memDonePtr);
        return;
    } 
    if(address == 0xFF){
        *dataPtr = 0;
        ...
        *memDonePtr = 1;
        ...
        if(missDone == 1){
            ...
            *memDonePtr = 1;
            ...
            memory->memFetch(computedCacheLine << 3, 8, dataArray, memDonePtr);
...
{
    if(!isActive){
        memory->memStore(address, count, dataPtr, memDonePtr);
        return;
    }   
...
void Cache::FFWrite(unsigned char *memDonePtr){
    ...
    for(int i = 0; i < 8; i++){
        if(validArray[i] == 2){
            memory->memStore((CLO << 3) + i, 1, dataArray + i, memDonePtr);
            hasWritable = 1;
            break;    
        }
    }
